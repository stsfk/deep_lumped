...
# %%
dcalibration = dataloader.Forcing_Data(
    "data/data_calibration_camels_de.csv",
    record_length=4017,
...
# %%
x_batch_calibration, y_batch_calibration = dcalibration.get_val_batch()
...
# %%
def fitting_wrapper(selected_catchment, batch_size=200):

    # Subsetting training, validation, and test data of selected catchments
    x_calibration = x_batch_calibration[:,selected_catchment,:,:].to(computing_device)
    y_calibration = y_batch_calibration[:,selected_catchment,:].to(computing_device)
    ...
    fn_calibration = Objective_builder_batch(x_calibration,y_calibration,HydroErr.kge_2009)
    ...
    ga_instance = pygad.GA(num_generations=num_generations,
                        ...
                        fitness_func=fn_calibration.eval,
...
# %%
np.savetxt("data/out_of_sample_results/camels_de_KGEs.csv", calibrated_KGES, delimiter=",")
np.savetxt("data/out_of_sample_results/camels_de_embeddings.csv", camels_embeddings, delimiter=",")
