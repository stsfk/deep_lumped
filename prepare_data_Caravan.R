...
data_process <- data_ts %>% 
  ...
  filter(full_catchment_id %in% non_US_catchments$gauge_id) %>%
...
data_train_val %>%
  ...
  select(P:Q) %>%
  write_csv(file = "./data/data_train_val_CARAVAN.csv")

data_train %>%
  arrange(catchment_id, date) %>%
  select(P:Q) %>%
  write_csv(file = "./data/data_train_CARAVAN.csv")

data_val %>%
  arrange(catchment_id, date) %>%
  select(P:Q) %>%
  write_csv(file = "./data/data_val_CARAVAN.csv")

data_test %>%
  arrange(catchment_id, date) %>%
  select(P:Q) %>% 
  write_csv(file = "./data/data_test_CARAVAN.csv")

data_process %>%
  arrange(catchment_id, date) %>%
  select(P:Q) %>% 
  write_csv(file = "./data/data_all_CARAVAN.csv")
