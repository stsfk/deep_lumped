...
minimal_required_Q_length = 365*8
...
data_train_val %>%
  ...
  select(P:Q) %>%
  write_csv(file = "./data/data_train_val_com.csv")

data_train %>%
  arrange(catchment_id, date) %>%
  select(P:Q) %>%
  write_csv(file = "./data/data_train_com.csv")

data_val %>%
  arrange(catchment_id, date) %>%
  select(P:Q) %>%
  write_csv(file = "./data/data_val_com.csv")

data_test %>%
  arrange(catchment_id, date) %>%
  select(P:Q) %>% 
  write_csv(file = "./data/data_test_com.csv")

data_process %>%
  arrange(catchment_id, date) %>%
  select(P:Q) %>%
  write_csv(file = "./data/Caravan/data_com.csv")
